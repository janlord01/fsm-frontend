...
<template>
  <q-page class="flex flex-center">
    <q-card class="q-pa-md shadow-1">
      ...
      <q-card-section>
        <div class="text-h6">Sign Up As Client</div>
      </q-card-section>
      <q-card-section>
        <q-input
          filled
...
const signUpWithGoogle = async () => {
   ...
   */
  const redirectUri = appUrl + "/register"; // google auth redirect
...
const signUpWithFacebook = async () => {
  ...
  const redirectUri = appUrl + "/register"; // Must match the URI in your Facebook app settings
...
const handleFacebookCallback = async () => {
  ...
  // Local
  const redirectUri = appUrl + "/register";

  if (code) {
    $q.loading.show({ message: "Please wait..." });

    await api
      .get("/api/auth/facebook/register/client", {
        params: {
          code,
          state,
          redirectUri,
...
const handleGoogleCallback = async () => {
  ...
  if (code) {
    $q.loading.show({ message: "Please wait..." });
    await api
      .get("/api/auth/google/register/client", {
        params: {
          code,
          state,
          scope,
